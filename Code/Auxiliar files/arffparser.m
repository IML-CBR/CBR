function out = arffparser(mode, fileName, normalize, inputStruct, relationName, ...
    cmmnts)
...
%   -----------------------------------------------------------------------
    ...
    if normalize == 1
        ...
        for field_index = 1:numel(fnames)
            if strcmp(out.(fnames{field_index}).kind, 'numeric')
                minelement = min(out.(fnames{field_index}).values);
                maxelement = max(out.(fnames{field_index}).values);
                val = (out.(fnames{field_index}).values - minelement) / (maxelement - minelement);
                out.(fnames{field_index}).values = val;
            end
        end
    end
end
